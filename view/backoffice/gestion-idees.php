      ...
      <main class="page">
        ...
        <div class="page-grid users-page">
          <section class="users-hero">
            <div class="users-header">
              ...
              <div class="users-actions">
                ...
                <a class="ghost-button" href="gestion-brainstormings.php">Brainstormings</a>
              </div>
            </div>
            ...
            <form class="users-filters" method="get" action="gestion-idees.php">
              ...
              <div class="filter-field">
                <label for="idea-status">Statut</label>
                <select id="idea-status" name="statut">
          ...
          <?php if ($feedbackMessage !== ''): ?>
            <section class="users-hero user-form-card">
              <div class="form-feedback <?= h($feedbackType) ?>"><?= h($feedbackMessage) ?></div>
            </section>
          <?php endif; ?>

          <?php if ($editIdea): ?>
            <section class="users-hero user-form-card" id="idea-form">
              <div>
                <h2 class="section-title">Modifier une idee</h2>
              </div>
              <form id="crud-idea-form" class="users-crud-form" method="post" action="gestion-idees.php?edit=<?= (int) $editIdea['id'] ?>" novalidate>
                <input type="hidden" name="action" value="update" />
                <input type="hidden" name="id" value="<?= (int) $editIdea['id'] ?>" />
                <div class="user-form-grid">
                  <div class="filter-field full-width">
                    <label for="contenu">Contenu *</label>
                    <textarea id="contenu" name="contenu" required><?= h((string) $editIdea['contenu']) ?></textarea>
                  </div>
                </div>
                <p id="crud-feedback" class="form-feedback"></p>
                <div class="users-actions">
                  <button class="action-button" type="submit">Enregistrer les modifications</button>
                  <a class="ghost-button" href="gestion-idees.php">Annuler</a>
                </div>
              </form>
            </section>
          <?php endif; ?>
            ...
            <table class="table users-table">
              ...
              <tbody>
                <?php if (count($ideas) === 0): ?>
                  <tr>
                    <td colspan="7">Aucune idee trouvee.</td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($ideas as $idea): ?>
                    ...
                    <tr>
                      ...
                      <td>
                        <span class="status-pill active"><?= (int) $idea['likes'] ?> likes</span>
                        <span class="status-pill risk"><?= (int) $idea['dislikes'] ?> dislikes</span>
                        <?php if (!empty($idea['is_winner'])): ?>
                          <span class="status-pill pending">Gagnant</span>
                        <?php endif; ?>
                      </td>
                      <td><?= h(formatDateTime((string) $idea['date_creation'])) ?></td>
                      <td>
                        <div class="table-actions">
                          <a class="ghost-button" href="../frontoffice/brainstorming-detail.php?id=<?= (int) $idea['brainstorming_id'] ?>">Voir</a>
                          <a class="ghost-button" href="gestion-idees.php?edit=<?= (int) $idea['id'] ?>#idea-form">Modifier</a>
                          <form class="inline-delete-form" method="post" action="gestion-idees.php" data-status-form data-status-label="approuver">
                          ...
                          <form class="inline-delete-form" method="post" action="gestion-idees.php" data-status-form data-status-label="desapprouver">
