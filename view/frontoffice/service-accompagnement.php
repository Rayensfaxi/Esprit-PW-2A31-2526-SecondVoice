...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_goal' || $action === 'update_goal') {
        ...
        if (empty($title) || empty($description) || empty($type) || $assistant_id <= 0) {
            // Validation error: show flash on the same page (don't redirect — keep typed values)
...
// Pre-fill values when arriving from Echo Me or SmartGuide
// (only on creation, never when editing an existing goal).
$prefillDesc = '';
$prefillType = '';
$prefillAssistant = 0;
...
<div class="page-wrapper">
  ...
  <?php if ($cameFromEchoMe): ?>
    <div class="echo-import-notice">
      <span class="echo-import-icon">🎙️✨</span>
      <div>
        <strong>Texte importé depuis Echo Me</strong> — Votre phrase clarifiée a été insérée dans la description ci-dessous.
        Vérifiez et complétez avant de soumettre.
      </div>
    </div>
  <?php endif; ?>
  ...
  <?php if ($cameFromSmartGuide): ?>
    <div class="echo-import-notice">
      <span class="echo-import-icon">🤖✨</span>
      <div>
        <strong>Recommandation SmartGuide</strong> — J'ai pré-rempli le <strong>type</strong>,
        l'<strong>assistant suggéré</strong><?= $prefillDesc ? ' et la <strong>description</strong>' : '' ?>
        d'après vos réponses. Vous pouvez tout modifier avant de soumettre.
      </div>
    </div>
  <?php endif; ?>
        ...
        <div class="form-grid">
          <div class="form-group">
            <div class="label-with-action">
              ...
              <button type="button" class="vd-btn" id="vd-title" title="Dicter le titre à voix haute">🎙️ Dicter</button>
            </div>
            <input
              type="text"
              id="title"
          ...
          <div class="form-group">
            <div class="label-with-action">
              ...
              <button type="button" class="vd-btn" id="vd-description" title="Dicter la description à voix haute">🎙️ Dicter</button>
            </div>
            <textarea
              id="description"
              name="description"
